...
const verification = {
  schema_version: 2,
  conclusion: 'stage1_rework_ready_for_technical_rereview',
  source: {
...
const blockerClosure = {
  schema_version: 1,
  reviewed_commit: 'ebe89781a92c3d199a68b0c0de623dc6470f6adb',
  replacement_commit: sourceCommit,
  items: [
    {
      severity: 'blocker',
      id: 'B1-explicit-keyword-confirmation',
      status: 'closed_by_rework_pending_review',
      implementation: [
        'local planner has no raw-token fallback',
        'renderer requires plan -> edit/confirm/cancel before run',
        'pipeline and online quota gate accept only confirmedKeywords'
      ],
      evidence: [
        'results/probe-tests.json',
        'results/smoke/smoke-report.json checks sensitive_fallback/all_unconfirmed/request/quota'
      ]
    },
    {
      severity: 'blocker',
      id: 'B2-image-trust-and-worker-isolation',
      status: 'closed_by_rework_pending_review',
      implementation: [
        'Content-Type and PNG/JPEG/WebP magic must match before Sharp',
        '10 MiB declared and streamed limits fail closed',
        'worker jobs have a fixed watchdog, kill/recreate, and queue recovery'
      ],
      evidence: [
        'source/ym-10-dor/fixtures/security-fixture-matrix.json',
        'results/probe-tests.json',
        'results/smoke/smoke-report.json security and worker recovery checks'
      ]
    },
    {
      severity: 'blocker',
      id: 'B3-license-and-maturity-fail-closed',
      status: 'closed_by_rework_pending_review',
      implementation: [
        'mature must be boolean false',
        'asset ID must be UUID and equal the fixed thumbnail-path ID',
        'CC0/PDM labels must match canonical Creative Commons license URLs'
      ],
      evidence: [
    ...
    {
      severity: 'major',
      id: 'M1-clean-commit-evidence-provenance',
      status: 'closed_by_rework_pending_review',
      implementation: [
        'this package was generated by verify-stage1.mjs from a clean exact commit',
        'npm ci precedes every command',
        'Git-blob LF and Windows-worktree lock hashes are separately named and verified',
        'test totals are parsed from this checkout test JSON, never hard-coded'
      ],
      evidence: [
        'environment.json',
        'commands.json',
        'verification.json',
        'results/build-result.json',
...
const manifest = {
  ...
  conclusion: 'rework_ready_for_technical_rereview',
  source: {
    commit: sourceCommit,
    parent: sourceParent,
