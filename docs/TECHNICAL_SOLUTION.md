...
Canvas 2D 生成双版式 PNG，标准 Unicode 生成器提供行内 Emoji，Node.js SQLite 保存历史、
收藏与偏好，electron-vite 负责开发和构建。
...
flowchart LR
    ...
    F --> Q{"输出类型"}
    V --> Q
    Q -->|"inline"| I["5 个行内 Unicode Emoji"]
    I --> T["文本剪贴板"]
    Q -->|"image"| G["生成参数"]
    G --> E["本地效果解析"]
    E --> D["双版式 Canvas 2D"]
...
图中模型与 CLI 只负责文本分析和方案规划；应用根据当前输出类型选择 Unicode 或 Canvas
单一路径，不同时生成两类结果。绘制、复制、导出、收藏与持久化仍由 EmojiPie 控制。
预加载桥只暴露白名单 IPC，渲染进程启用上下文隔离并关闭 Node.js 集成。
...
图片输出时，界面提供 `smart` 生成选择，以及 `classic`、`cute`、`deadpan`、`office`、`sarcastic`、
...
图片候选区固定为 9 张，不再通过 `IntersectionObserver` 自动追加。“换一批”捕获上一批的
文案、模式、效果选择与渲染设置，重新执行完整生成流程；旧候选在成功前保持可用，失败
时不会清空，成功后整批替换。新旧图片批次均保存在 SQLite 历史记录中。

## 行内 Emoji 与客户端兼容

每批根据同一份 `TextAnalysis` 确定性生成 5 个 Unicode 候选，包括单个情绪 Emoji 与最多
两个码点的“情绪 + 场景”组合。候选只使用具备默认 `Emoji_Presentation` 的成熟字符，不输出
平台私有短码、ZWJ 序列或变体选择符；显示样式和行高由微信、飞书、钉钉等目标客户端决定。

点击候选通过 `clipboard:write-text` 写入文本剪贴板，主进程再次验证 1 至 2 个 Unicode
图形码点。行内模式只展示这 5 个候选，不渲染图片，也不写入图片历史与收藏库。“换一批”
只替换当前行内候选。该能力不增加模型输出字段；运行时仍返回九项协议，界面只消费其中的
`TextAnalysis`。

## 输出类型、双版式与数据兼容

生成设置由 `EmojiRenderSettings` 表示，包含 `outputType: 'inline' | 'image'`、
`layout: 'compact' | 'poster'` 与 `embedCaption: boolean`。界面将其映射为“行内 Emoji / 黄脸
贴纸 / 表情海报”三个并列选项，但内部保留文本与图片两类输出边界。新用户和缺少
`outputType` 的旧偏好均归一化为 `image + compact + false`。设置经类型化 IPC 保存到 SQLite
`preferences`；浏览器预览使用 `localStorage`。生成和换批均使用不可变设置快照。

- **黄脸贴纸**：先在 `640x640` 透明画布绘制，经 `256x256` 中间画布后高质量缩放为
  `128x128` PNG。无字模式下
...
图片历史再次创作时强制恢复为 `outputType: 'image'`；行内结果不需要数据库迁移。
...
- Playwright 启动真实 Electron，验证三种互斥输出、行内 Emoji 文本剪贴板且不入图片库、
  图片效果缩略图、候选换批与历史保留、图片复制、收藏、双版式像素输出、设置恢复、两类
  运行时设置和 960/1320 布局。
